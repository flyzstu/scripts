...
# ========== 检查并创建 Systemd 单元文件 ==========
echo -e "${BLUE}检查 systemd 单元文件...${NC}"
SYSTEMD_FILE="/etc/systemd/system/$SERVICE_NAME.service"

if [ ! -f "$SYSTEMD_FILE" ]; then
    echo -e "${YELLOW}systemd 单元文件不存在，正在创建...${NC}"
    
    # 检查 root 权限
    if [ "$EUID" -ne 0 ]; then
        echo -e "${RED}错误：需要 root 权限来创建 systemd 单元文件。${NC}"
        exit 1
    fi
...
EOF
    ...
    # 移动到正确位置
    mv "/tmp/$SERVICE_NAME.service" "$SYSTEMD_FILE"
    chmod 644 "$SYSTEMD_FILE"
    systemctl daemon-reload
    echo -e "${GREEN}systemd 单元文件创建成功。${NC}"
else
    echo -e "${GREEN}systemd 单元文件已存在。${NC}"
fi
# ================================================
